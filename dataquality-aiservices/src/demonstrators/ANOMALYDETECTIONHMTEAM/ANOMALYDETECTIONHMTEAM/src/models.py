...
class TranAD(nn.Module):
	def __init__(self, feats):
		...
		self.n = self.n_feats * self.n_window
		self.pos_encoder = PositionalEncoding(2 * feats, 0.1, self.n_window)
		encoder_layers = TransformerEncoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, batch_first=True, dropout=0.1)
		self.transformer_encoder = TransformerEncoder(encoder_layers, 1)
		decoder_layers1 = TransformerDecoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, batch_first=True, dropout=0.1)
		self.transformer_decoder1 = TransformerDecoder(decoder_layers1, 1)
		decoder_layers2 = TransformerDecoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, batch_first=True, dropout=0.1)
		self.transformer_decoder2 = TransformerDecoder(decoder_layers2, 1)
		self.fcn = nn.Sequential(nn.Linear(2 * feats, feats), nn.Sigmoid())
...
class TranAD3(nn.Module):
    def __init__(self, feats, lr=1e-3):
        super(TranAD, self).__init__()
        self.name = 'TranAD'
        self.lr = lr
        self.batch = 128
        self.n_feats = feats
        self.n_window = 10
        d_model = 2 * feats  # Embedding dimension should be consistent across layers
        
        # Positional encoding
        self.pos_encoder = PositionalEncoding(d_model, 0.1, self.n_window)
        
        # Transformer encoder and decoder layers
        encoder_layers = TransformerEncoderLayer(d_model=d_model, nhead=feats, dim_feedforward=16, dropout=0.1, batch_first=True)
        self.transformer_encoder = TransformerEncoder(encoder_layers, num_layers=1)
        
        decoder_layers1 = TransformerDecoderLayer(d_model=d_model, nhead=feats, dim_feedforward=16, dropout=0.1, batch_first=True)
        self.transformer_decoder1 = TransformerDecoder(decoder_layers1, num_layers=1)
        
        decoder_layers2 = TransformerDecoderLayer(d_model=d_model, nhead=feats, dim_feedforward=16, dropout=0.1, batch_first=True)
        self.transformer_decoder2 = TransformerDecoder(decoder_layers2, num_layers=1)
    ...
    def forward(self, src, tgt):
        # Phase 1 - Without anomaly scores
        c = torch.zeros_like(src)
        tgt, memory = self.encode(src, c, tgt)
